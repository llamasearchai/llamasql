...
from rag_example import Document, RAGSystem, load_sample_knowledge_base
...
def query():
    ...
    try:
        ...
        response, contexts = rag.generate_response(
            query=query_text, k=data.get("k", 3), filter=filter_data
        )
        ...
        return jsonify({"response": response, "contexts": formatted_contexts})
...
if __name__ == "__main__":
    ...
    # Start the server
    app.run(
        host="0.0.0.0", port=8000, debug=os.getenv("DEBUG", "False").lower() == "true"
    )
