...
from llamadb.integrations.anthropic import (
    AnthropicClient,
    ClaudeEmbeddings,
    ClaudeRAGPipeline,
)
...
def check_api_key():
    ...
    if not api_key:
        console.print(
            "[bold red]Error:[/bold red] ANTHROPIC_API_KEY environment variable not found."
        )
...
def demo_embeddings(api_key):
    ...
    # Display results
    console.print(
        f"Generated {len(texts)} embeddings in {(end_time - start_time)*1000:.2f}ms"
    )
    ...
    for i in range(len(texts)):
        for j in range(i + 1, len(texts)):
            ...
            table.add_row(
                texts[i][:30] + "...", texts[j][:30] + "...", f"{similarity:.4f}"
            )
...
def main():
    ...
    parser.add_argument(
        "--api-key", help="Anthropic API key (overrides environment variable)"
    )
    ...
    if not api_key:
        ...
        console.print(
            "Please provide an API key with --api-key or set the ANTHROPIC_API_KEY environment variable."
        )
        return 1
