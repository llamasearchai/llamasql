...
try:
    ...
    try:
        ...
        console.print(f"[bold red]Error:[/bold red] Could not import LlamaDB components: {e2}")
        console.print("[yellow]Please ensure LlamaDB is installed correctly with:[/yellow]")
...
def print_header():
    ...
    subtitle = Text("A Next-Gen Hybrid Python/Rust Data Platform with MLX Acceleration", style="italic cyan")
...
def show_system_info():
    ...
    table.add_row("Apple Silicon", f"[green]Yes[/green]" if apple_silicon else "[yellow]No[/yellow]")
    table.add_row("MLX Available", f"[green]Yes[/green]" if mlx_available else "[yellow]No[/yellow]")
...
def demo_vector_operations():
    ...
    try:
        ...
        table.add_row(
            ...
            f"{numpy_time * 1000000:.2f}",
            "NumPy"
        )
        
        console.print(table)
        console.print("[yellow]Note: MLX acceleration for vector operations with 1D arrays is currently showing warnings.[/yellow]")
        console.print("[yellow]This is a known issue that will be fixed in a future release.[/yellow]")
    ...
    try:
        ...
        try:
            ...
            if 'numpy_time' in results and results['numpy_time'] > 0:
                ...
                console.print("[yellow]Invalid benchmark results. Performing manual benchmark...[/yellow]")
                perform_manual_matrix_benchmark(matrix_size)
        except Exception as inner_e:
            console.print(f"[yellow]Error in benchmark function: {str(inner_e)}[/yellow]")
...
def perform_manual_matrix_benchmark(size=1000):
    ...
    if is_mlx_available():
        try:
            ...
            console.print(f"[yellow]Error using MLX for matrix multiplication: {e}[/yellow]")
...
def demo_api_compatibility():
    ...
    try:
        ...
        console.print(f"- Original small vector norm: {np.linalg.norm(small_vector):.4f}")
        console.print(f"- Normalized small vector norm: {np.linalg.norm(normalized_small):.4f}")
        console.print(f"- Padded vector norm: {np.linalg.norm(padded_vector):.4f}")
        console.print(f"- Normalized padded vector norm: {np.linalg.norm(normalized_padded):.4f}")
        ...
        console.print("4. For best results, normalize vectors before padding/truncating")
...
def main():
    ...
    try:
        ...
        console.print("For more features, try running: [bold]python -m llamadb.cli.main demo[/bold]")
    except Exception as e:
        console.print(f"[bold red]An error occurred during the demo:[/bold red] {str(e)}")
