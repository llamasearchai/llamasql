    ...
    async def search_vectors(request: VectorSearchRequest):
        ...
        try:
            ...
            if query.shape[0] != _vector_index.dimension:
                logger.warning(
                    f"Query vector dimension ({query.shape[0]}) does not match index dimension ({_vector_index.dimension})"
                )
                if query.shape[0] < _vector_index.dimension:
                    # Pad vector with zeros to match dimension
                    padding = np.zeros(
                        _vector_index.dimension - query.shape[0], dtype=np.float32
                    )
                    query = np.concatenate([query, padding])
                    logger.info(
                        f"Vector padded from {len(request.vector)} to {_vector_index.dimension} dimensions"
                    )
                else:
                    # Truncate vector to match dimension
                    query = query[: _vector_index.dimension]
                    logger.info(
                        f"Vector truncated from {len(request.vector)} to {_vector_index.dimension} dimensions"
                    )
            ...
            # Perform search
            start = time.time()
            results = _vector_index.search(
                query, k=request.k, filter_metadata=filter_metadata
            )
            ...
            if "dimension" in error_str and "does not match" in error_str:
                ...
                logger.warning(
                    f"Dimension mismatch error handled gracefully: {error_str}"
                )
                return {
                    "results": [],
                    "took_ms": 0,
                    "backend": (
                        "MLX" if _vector_index and _vector_index.use_mlx else "NumPy"
                    ),
...
def create_demo_index(dimension: int = 128, num_vectors: int = 10000) -> VectorIndex:
    ...
    """
    logger.info(
        f"Creating demo vector index with {num_vectors} vectors of dimension {dimension}"
    )
