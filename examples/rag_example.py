...
class RAGSystem:
    ...
    def _chunk_document(self, document: Document) -> List[Chunk]:
        ...
        while start < len(content):
            ...
            if start > 0 and end < len(content):
                ...
                sentences_end = [m.end() for m in re.finditer(r'[.!?]\s', content[start:end])]
                if sentences_end:
    ...
    def save(self, file_path: str) -> None:
        ...
        print(f"Index saved successfully. Current index has {self.index.count()} chunks.")
...
def load_sample_knowledge_base() -> List[Document]:
    ...
    documents = [
        ...
        Document(
            ...
            metadata={"category": "Natural Language Processing", "difficulty": "advanced"}
        ),
        Document(
            id="doc4",
            ...
            metadata={"category": "Reinforcement Learning", "difficulty": "intermediate"}
        ),
        Document(
            id="doc5",
...
def main():
    ...
    response, contexts = rag.generate_response(
        query=query,
        k=2,
        filter={"category": "Natural Language Processing"}
    )
