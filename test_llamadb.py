...
parser.add_argument(
    "--dimensions", type=int, default=1536, help="Vector dimensions for tests"
)
parser.add_argument(
    "--count", type=int, default=10000, help="Number of vectors for tests"
)
...
def test_vector_operations() -> None:
    ...
    print_result(
        "Cosine Similarity",
        cos_sim_correct,
        f"Got {cos_sim:.6f}, Expected {expected_cos:.6f}",
    )
    ...
    print_result(
        "Dot Product", dot_correct, f"Got {dot:.6f}, Expected {expected_dot:.6f}"
    )
...
def run_benchmarks() -> Dict[str, Any]:
    ...
    if args.count >= 100:
        ...
        matrix_b = (
            vectors[subset_size : subset_size * 2].T
            if subset_size * 2 <= args.count
            else vectors[:subset_size].T
        )
        ...
        print(
            f"Matrix Multiply ({subset_size}x{args.dimensions} * {args.dimensions}x{subset_size}): {matrix_time:.4f} seconds"
        )
