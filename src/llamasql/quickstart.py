...
try:
    ...
    try:
        ...
        print(
            "Make sure LlamaDB is properly installed or run this from the project root directory."
        )
        sys.exit(1)
    ...
    for i in range(count):
        ...
        doc = {
            ...
            "created_at": time.strftime(
                "%Y-%m-%d",
                time.localtime(time.time() - np.random.randint(0, 365 * 24 * 3600)),
            ),
            "embedding": embedding,
        }
...
def run_quickstart_demo() -> None:
    ...
    print(
        f"- MLX Acceleration: {'Available' if is_mlx_available() else 'Not available'}"
    )
    ...
    for i, result in enumerate(results):
        print(
            f"  {i+1}. Document {result['id']} (Score: {result.get('score', 'N/A'):.4f})"
        )
    ...
    filtered_results = [
        result for result in all_results if result.get("category") == filter_category
    ][
        :5
    ...
    for i, result in enumerate(filtered_results):
        print(
            f"  {i+1}. {result.get('title', 'Untitled')} - {result.get('category', 'Uncategorized')}"
        )
