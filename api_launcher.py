...
def kill_process(pid: int) -> bool:
    ...
    try:
        ...
        try:
            os.kill(pid, 0)  # Doesn't actually send a signal, just checks if process exists
...
def start_server(port: int = DEFAULT_PORT, host: str = DEFAULT_HOST, restart: bool = False) -> None:
    ...
    if is_port_in_use(port, host):
        ...
        if restart:
            ...
            if pid:
                ...
                print(f"Could not find process using port {port}. Please free the port manually.")
                return
        else:
    ...
    try:
        cmd = [sys.executable, "-m", "llamadb.cli.main", "serve", "--host", host, "--port", str(port)]
...
def main() -> None:
    ...
    group.add_argument("--status", action="store_true", help="Check if server is running")
    ...
    if args.status:
        if is_port_in_use(args.port, args.host):
            ...
            print(f"LlamaDB API server is running on {args.host}:{args.port}" + (f" (PID: {pid})" if pid else ""))
        else:
