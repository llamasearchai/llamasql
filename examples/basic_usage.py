...
def main():
    ...
    with connect(f"sqlite:///{db_path}") as conn:
        ...
        select_query = (
            Query()
            .select("u.name AS author", "p.title", "p.content", "p.created_at")
            .from_table("posts AS p")
        ...
        select_query = (
            Query()
            .select("u.name", Query.func.count("p.id").as_("post_count"))
            .from_table("users AS u")
        ...
        update_query = (
            Query()
            .update(posts_table.name)
            .set({"published": True})
            .where("title = ?", ["Alice's Second Post"])
        )
        ...
        # Query: Delete a user
        delete_query = (
            Query().delete_from(users_table.name).where("name = ?", ["Charlie"])
        )
        ...
        select_query = (
            Query().select("name").from_table(users_table.name).order_by("name")
        )
        sql, params = select_query.build()
        result = conn.execute(sql, params)
