...
from llamasql.schema import Table, Column, ColumnReference
...
class Condition:
    ...
    def __str__(self) -> str:
        ...
        if self.operator == "BETWEEN" and isinstance(self.right, tuple) and len(self.right) == 2:
...
class CompoundCondition:
    ...
    """
    def __init__(self, conditions: List[Union[Condition, 'CompoundCondition']], operator: str):
    ...
    # Logical operators for combining conditions
    def __and__(self, other: Union[Condition, 'CompoundCondition']) -> 'CompoundCondition':
        if not isinstance(other, (Condition, CompoundCondition)):
            raise TypeError("AND operator only works with Condition or CompoundCondition objects")
    ...
    def __or__(self, other: Union[Condition, 'CompoundCondition']) -> 'CompoundCondition':
        if not isinstance(other, (Condition, CompoundCondition)):
            raise TypeError("OR operator only works with Condition or CompoundCondition objects")
...
class Query:
    ...
    def right_join(self, table: Union[str, Table], on: Union[Condition, str]) -> 'Query':
        """
    ...
    def sql(self) -> Tuple[str, list]:
        ...
        if self._having_conditions:
            having_conditions_str = " AND ".join(str(c) for c in self._having_conditions)
