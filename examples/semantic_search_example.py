...
class SemanticSearchEngine:
    ...
    def search(
        ...
        # Search the index
        results = self.index.search(query_vector=query_embedding, k=k, filter=filter)
...
def main():
    ...
    results = search_engine.search(
        "advanced technology trends", k=3, filter={"category": "Technology"}
    )
    for i, result in enumerate(results, 1):
        print(f"{i}. {result['title']} (Score: {result['score']:.4f})")
        print(f"   Category: {result['category']}, Date: {result['date']}")
    ...
    results = search_engine.search(
        ...
        k=3,
        filter={
            "$or": [{"category": "Technology"}, {"category": "Machine Learning"}],
            "date": {"$gte": "2023-06-01"},
        },
    )
    for i, result in enumerate(results, 1):
