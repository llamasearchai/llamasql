...
class Migration:
    ...
    def create_table(self, table_name: str, columns: Dict[str, str], if_not_exists: bool = False):
        """
        ...
        sql = f"CREATE TABLE {exists_clause}{table_name} (\n  " + ",\n  ".join(column_defs) + "\n)"
    ...
    def create_index(self, table_name: str, column_names: List[str], index_name: str = None, unique: bool = False):
        """
        Create an index.
        ...
        self.up(f"CREATE {unique_clause}INDEX {index_name} ON {table_name} ({', '.join(column_names)})")
...
class Migrator:
    ...
    def get_applied_migrations(self) -> List[Dict]:
        ...
        """
        result = self.connection.execute(f"SELECT * FROM {self.migrations_table} ORDER BY id")
    ...
    def _get_next_batch(self) -> int:
        ...
        Returns:
            Next batch number
        """
        result = self.connection.execute(f"SELECT MAX(batch) as max_batch FROM {self.migrations_table}")
        row = result.first()
