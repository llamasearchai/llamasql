...
def cosine_similarity(
    query: np.ndarray, vectors: np.ndarray, use_mlx: bool = True
) -> np.ndarray:
    """
    ...
    if use_mlx and is_mlx_available():
        try:
            ...
            # Normalize vectors
            vectors_norm = mx.sqrt(
                mx.sum(vectors_mx * vectors_mx, axis=1, keepdims=True)
            )
            vectors_norm = mx.where(
                vectors_norm == 0, mx.ones_like(vectors_norm), vectors_norm
            )
            ...
            logger.warning(
                f"Error using MLX for cosine similarity: {e}. Falling back to NumPy."
            )
...
def l2_distance(
    ...
    if use_mlx and is_mlx_available():
        try:
            ...
            logger.warning(
                f"Error using MLX for L2 distance: {e}. Falling back to NumPy."
            )
...
def dot_product(
    ...
    if use_mlx and is_mlx_available():
        try:
            ...
            logger.warning(
                f"Error using MLX for dot product: {e}. Falling back to NumPy."
            )
...
def matrix_multiply(a: np.ndarray, b: np.ndarray, use_mlx: bool = True) -> np.ndarray:
    """
    ...
    if use_mlx and is_mlx_available():
        try:
            ...
            logger.warning(
                f"Error using MLX for matrix multiplication: {e}. Falling back to NumPy."
            )
...
def benchmark_matrix_multiply(size: int = 1000, iterations: int = 5) -> dict:
    """
    ...
    results = {"numpy_time": numpy_time, "numpy_gflops": numpy_gflops}
