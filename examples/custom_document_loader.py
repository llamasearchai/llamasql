...
from rag_example import RAGSystem, Document, Chunk
...
try:
    ...
    print("Web scraping support not available. Install required packages with: pip install requests beautifulsoup4")
...
class WebLoader(DocumentLoader):
    ...
    def __init__(self, metadata: Optional[Dict[str, Any]] = None):
        ...
        if not HAS_WEB_SUPPORT:
            raise ImportError("Web scraping requires requests and beautifulsoup4 libraries")
        self.metadata = metadata or {}
    ...
    def load(self, source: str) -> List[Document]:
        ...
        try:
            ...
            main_elements = soup.find_all(['article', 'main', 'div'], class_=re.compile(r'content|main|article'))
        ...
        try:
            ...
            if self.items_key:
                if isinstance(data, dict) and self.items_key in data:
                    ...
                    raise ValueError(f"Items key '{self.items_key}' not found in JSON data")
            ...
            for idx, item in enumerate(items):
                ...
                if isinstance(item, dict) and self.content_key in item:
                    ...
                    print(f"Skipping item {idx}: content key '{self.content_key}' not found")
                    continue
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Load custom documents into the LlamaDB RAG system")
    parser.add_argument("--source", required=True, help="Source to load documents from (file path, URL, etc.)")
    parser.add_argument("--type", required=True, choices=["pdf", "web", "csv", "markdown", "json"], help="Type of document source")
    parser.add_argument("--save-path", default="knowledge_base.llamadb", help="Path to save the updated index")
    parser.add_argument("--chunk-size", type=int, default=600, help="Chunk size for splitting documents")
    parser.add_argument("--chunk-overlap", type=int, default=100, help="Chunk overlap for splitting documents")
    
    # CSV/Excel specific arguments
    parser.add_argument("--text-columns", help="Comma-separated list of columns to use for text content (CSV/Excel only)")
    parser.add_argument("--id-column", help="Column to use as document ID (CSV/Excel only)")
    parser.add_argument("--title-column", help="Column to use as document title (CSV/Excel only)")
    parser.add_argument("--metadata-columns", help="Comma-separated list of columns to add as metadata (CSV/Excel only)")
    
    # JSON specific arguments
    parser.add_argument("--content-key", default="content", help="JSON key for document content (JSON only)")
    parser.add_argument("--items-key", help="JSON key for list of items to process (JSON only)")
    ...
    if args.type == "pdf":
        ...
        metadata_columns = args.metadata_columns.split(",") if args.metadata_columns else []
